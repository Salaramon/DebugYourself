...
namespace stl {
	...
	struct TemplateTupleElement<I, TemplateTuple<Head, Tail...>>
		: public TemplateTupleElement<I - 1, std::tuple<Tail...>> { };
	...
	struct IsNotSame : public std::true_type {};

	template<class T>
	struct IsNotSame<T, T> : public std::false_type {};
	...
	template<>
	struct _Not<true> : public std::false_type {};

	template<>
	struct _Not<false> : public std::true_type {};
	...
	struct NonTypeTupleElement<I, NonTypeTuple<Type, Head, Tail...>>
		: public NonTypeTupleElement<I - 1, NonTypeTuple<Type, Tail...>> { };
	...
	*/
	...
	template<template<class...> class Template, class Tuple1, class Tuple2>
	struct _ConcatenateTuples {};

	template<template<class...> class Template, template<class...> class Template1, class... Args1, template<class...> class Template2, class... Args2>
	struct _ConcatenateTuples<Template, Template1<Args1...>, Template2<Args2...>> {
	...
	template<template<class...> class Template, class Tuple, class... RestTuples>
	struct ConcatenateTuples<Template, Tuple, RestTuples...> {
		using type = _ConcatenateTuples<Template, Tuple, typename ConcatenateTuples<Template, RestTuples...>::type>;
	};

	template<template<class...> class Template, class Tuple1, class Tuple2>
	struct ConcatenateTuples {
		using type = _ConcatenateTuples<Template, Tuple1, Tuple2>::type;
	};
	...
	template<template<class...> class Template, class... Args>
	struct TupleContainer<Template<Args...>> {
	public:
		inline static constexpr size = sizeof...(Args);
		using begin = Template<Args...>;
		using end = Template<>;
	};
	...
	template<template<class...> class Template, class T, class... Args>
	struct TupleIterator<Template<T, Args...>> {
		using type = T;
		using next = TupleIterator<Template<Args>>;
	};

	template<bool condition>
	struct Condition {
		inline static constexpr value = condition;
	};
	...
	template<class ConditionType, class True, class False>
	struct If : public _If<ConditionType::value, True, False> {};
	...
	template<size_t From, size_t To, template<class...> class Template, class T, class... Args>
	struct CutTuple<From, To, Template<T, Args...>> {
	public:
		//using type = CutTuple<From+1, To,
	};

	template<size_t From, template<class...> class Template, class T, class... Args>
	struct CutTuple<From, From, Template<T, Args...>> {
	public:
		//using type =
	};
	...
	template<size_t index, template<class...> class Template, class... Args>
	struct SplitTupleAt<index, Template<Args...>> {
	public:
		using type = int;
	};
	...
	template<class T, template<class...> class Template, class... Args>
	struct GetIndexList<T, Template<Args...>> {
	public:
